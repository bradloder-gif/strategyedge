...
export async function insertLeadSubmission(submission: InsertLeadSubmission): Promise<void> {
  const db = await getDb();
  if (!db) {
    console.warn("[Database] Cannot insert lead submission: database not available");
    return;
  }
